...
class SaleFollowupLine(models.Model):
    _name = 'sale_credit.followup.line'
    ...
    def _get_next_followup(self):
        self.ensure_one()
        return self.env['sale_credit.followup.line'].search([('delay', '>', self.delay), ('company_id', '=', self.env.company.id)], order="delay asc", limit=1)

class SaleCreditLine(models.Model):
    _inherit = 'sale.credit.line'

    followup_line_id = fields.Many2one('sale_credit.followup.line', 'Follow-up Level', copy=False)
    ...
    invoice_date = fields.Date(string='fecha de pago')
    invoice_origin = fields.Char(string='fecha esperada de pago')
